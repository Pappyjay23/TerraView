	...
	return (
		<aside
			className={`w-18 md:w-42 lg:w-64 p-4 flex flex-col rounded-r-2xl border-r transition-all duration-500 
                ${
									isDarkMode
										? "bg-gray-900/80 backdrop-blur-md shadow-lg border-gray-700"
			...
			<div className='flex flex-col lg:flex-row items-center gap-3 mb-8'>
				...
				<div className='text-center lg:text-left'>
					<h3
						className={`font-medium text-sm md:text-base md:hidden  ${
							isDarkMode ? "text-white" : "text-black"
						}`}>
						Sarah
					</h3>
					<h3
						className={`font-medium text-sm md:text-base hidden md:flex  ${
							isDarkMode ? "text-white" : "text-black"
						}`}>
						Sarah Connor
					</h3>
					<p
						className={`text-[70%] md:text-sm transition-colors duration-500 
			...
			{/* Bottom Actions */}
			<div
				className={`border-t pt-4 space-y-2 transition-colors duration-500 
				...
				<button
					className={`flex items-center gap-3 px-3 py-2 rounded-lg w-full transition-all duration-500 ease-in-out cursor-pointer
                        ${
													isDarkMode
														? "text-gray-400 hover:text-gray-100 hover:bg-red-500"
